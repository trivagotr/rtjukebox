...
async function fetchRssPodcast(feedUrl: string): Promise<Podcast[]> {
    try {
        ...
        return feed.items.map(item => ({
            ...
            date: item.published ? new Date(item.published).toLocaleDateString('tr-TR') : '',
...
async function fetchWebPodcasts(page: number): Promise<Podcast[]> {
    try {
        ...
        for (const article of articles) {
            const titleMatch = article.match(/<h[234][^>]*class="[^"]*entry-title[^"]*"[^>]*>[\s\S]*?<a[^>]*href="([^"]*)"[^>]*>([^<]*)<\/a>/i);
            if (!titleMatch) continue;

            const podcastUrl = titleMatch[1];
            const title = titleMatch[2].trim();

            const dateMatch = article.match(/<time[^>]*datetime="([^"]*)"[^>]*>([^<]*)<\/time>/i);
            ...
            const id = podcastUrl.split('/').filter(Boolean).pop() || `podcast-${podcasts.length}`;
...
export async function fetchSpotifyUrl(podcastUrl: string): Promise<string | null> {
    try {
        ...
        const spotifyMatch = html.match(/href="(https:\/\/podcasters\.spotify\.com\/[^"]*)"/i);
