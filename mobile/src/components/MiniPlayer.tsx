...
import TrackPlayer, { usePlaybackState, State, useActiveTrack, useTrackPlayerEvents, Event } from 'react-native-track-player';
...
const MiniPlayer = () => {
    ...
    const isOnRadioTab = useNavigationState(state => {
        if (!state) return true; // Default to hiding if state not ready
        try {
            // Get the active route in the stack (should be 'Main')
            const stackRoute = state.routes[state.index] as any;
            if (stackRoute.name === 'Profile') return false; // Not on radio, but on profile
    ...
    useTrackPlayerEvents([Event.PlaybackMetadataReceived], async (event) => {
        console.log('[MiniPlayer] PlaybackMetadataReceived event:', JSON.stringify(event));
        if (event.type === Event.PlaybackMetadataReceived) {
            ...
            console.log('[MiniPlayer] Parsed metadata - title:', title, 'artist:', artist);
            ...
            if (currentTrackData?.id && title) {
                ...
                // Fetch artwork
                const fetchedArtwork = await fetchAlbumArtwork(`${songArtist} ${songTitle}`);
                if (fetchedArtwork) {
    ...
    if ((!displayTrack && !isChangingChannel) || isOnProfileScreen || isOnRadioTab) {
        return null;
    }

    const skipToPrevious = async () => {
        console.log('[MiniPlayer] skipToPrevious called. TrackID:', displayTrack?.id);

        let prevIndex = activeChannels.length - 1; // Default to last channel

        if (displayTrack?.id) {
            const currentIndex = activeChannels.findIndex(c => c.id === displayTrack.id);
            if (currentIndex !== -1) {
                prevIndex = (currentIndex - 1 + activeChannels.length) % activeChannels.length;
            } else {
                console.log('[MiniPlayer] Current track not in active list, defaulting to last active.');
            }
        } else {
            console.log('[MiniPlayer] No active track ID, defaulting to last active.');
        }
    ...
    const skipToNext = async () => {
        ...
        if (displayTrack?.id) {
            ...
            if (currentIndex !== -1) {
                ...
                console.log('[MiniPlayer] Current track not in active list, defaulting to first active.');
            }
        } else {
            console.log('[MiniPlayer] No active track ID, defaulting to first active.');
        }
