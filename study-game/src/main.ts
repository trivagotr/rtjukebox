...
if (mode === 'engine-proof') {
  ...
  ui.innerHTML = `
    <header class="study-hud" data-testid="study-hud">
    ...
    <nav class="hud-action-rail" aria-label="Study actions">
      <button id="wardrobe-toggle" data-testid="wardrobe-toggle" class="hud-action" type="button" aria-label="Open wardrobe" title="Open wardrobe" aria-expanded="false" aria-controls="wardrobe-panel"><span>LOOK</span></button>
      <button id="chat-toggle" data-testid="chat-toggle" class="hud-action" type="button" aria-label="Open chat" title="Open chat" aria-expanded="false" aria-controls="chat-dock"><span>CHAT</span></button>
    </nav>
    ...
    <footer id="chat-dock" data-testid="chat-dock" class="chat-dock" hidden>
  ...
  const chatDock = document.querySelector<HTMLElement>('#chat-dock')!
  ...
  const setWardrobeOpen = (open: boolean) => {
    ...
    toggle.setAttribute('aria-label', open ? 'Close wardrobe' : 'Open wardrobe')
  }
  ...
  chatToggle.addEventListener('click', () => setChatOpen(chatDock.hasAttribute('hidden')))
