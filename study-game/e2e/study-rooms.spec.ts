...
test('keeps the room as the primary surface with compact HUD drawers', async ({ page }) => {
  await page.goto('/')
  await expect(page.locator('html')).toHaveAttribute('data-study-ready', 'true', { timeout: 30_000 })

  await expect(page.getByTestId('study-hud')).toBeVisible()
  await expect(page.getByTestId('wardrobe-toggle')).toHaveAttribute('aria-label', 'Open wardrobe')
